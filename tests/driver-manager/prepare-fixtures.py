...
SOURCE_LOCK = json.loads((REPO / "sources.lock.json").read_text())
ASSETS = ROOT / "app/src/main/assets"
CORPUS = ASSETS / "fixtures"
CORPUS.mkdir(parents=True, exist_ok=True)
GENERATED = ROOT / "generated"
GENERATED.mkdir(exist_ok=True)
CACHE = Path(os.environ.get("DRIVER_TEST_CACHE_DIR", REPO / ".pocket-build/downloads"))
...
def driver_archive(kind):
    ...
    candidates = [CACHE / record["filename"], REPO / ".pocket-build/downloads" / record["filename"]]
    if kind == "t30":
        expected = SOURCE_LOCK["downloads"]["bundled_turnip"]
        if any(record[key] != expected[key] for key in ("url", "sha256", "bytes")):
            raise SystemExit("T30 test pin differs from the production source lock")
        candidates.append(REPO / SOURCE_LOCK["bundled_turnip"]["destination"])
    ...
    request = urllib.request.Request(record["url"], headers={"User-Agent": "Skate3Pocket-driver-tests/1.0"})
    try:
...
def compile_elf_fixture():
    ndk_version = SOURCE_LOCK["toolchain"]["ndk"]
