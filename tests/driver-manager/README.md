...
`prepare-fixtures.py` first checks the production download cache at
`.pocket-build/downloads`. Set `DRIVER_TEST_CACHE_DIR` to use another cache.
Missing T30/R7 release ZIPs are downloaded from the exact public URLs in
`fixtures.lock.json`, with mandatory size and SHA-256 verification. The T30 pin
must agree with the production source lock. No driver ZIP is committed here.
...
The original workspace harness passed **25 cases and 428 assertions** on a
Retroid Pocket 6 running Android 13 (API 33). That run tested these source hashes:
...
This is historical evidence, not a claim that this portable project has already
run on your device. The port retains the test source and cases; it regenerates
the small structural ELF instead of depending on a workstation's staged native
binary. Use a fresh device report to establish results for a new build.
