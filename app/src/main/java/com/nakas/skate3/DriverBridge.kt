...
/** Driver selection is fixed for a process, before the official engine loads. */
object DriverBridge {
    ...
    @Synchronized fun selected(context: Context): String {
        ...
        if (!prefs.contains("selected")) {
            check(prefs.edit().putString("selected", T30).commit()) { "Could not save the default GPU driver." }
        }
        // Preserve an unavailable selection so a missing import is visible;
        // never silently substitute a different driver.
        return prefs.getString("selected", T30) ?: T30
    }
    ...
    fun choices(context: Context): List<DriverOption> = buildList {
        add(DriverOption(T30, "Turnip T30", "Bundled · MrPurple", false))
        add(DriverOption(SYSTEM, "System GPU driver", "Built into your device", false))
